function Topology(ele){
    ...
    var w=ele.clientWidth,
        ...
        self=this;
    this.force = d3.layout.force().gravity(0.65).distance(200).charge(-3000).size([w, h]);
...
function creatDiv(obj)
{
		...
		success:function(data){
			...
			for(i=0;i<value.tableSize;i++)
			{
				...
				if(row.portDp==obj.id)
				{
				    ...
				    portRate=new Array();//由于data中只能传数组，将portRate定义为数组。
					portNumber=row.portNumber;
					rate=row.portRate;
					portRate.push(rate);
					portRates.push(portRate);
					...
					$.ajax(
						...
						success:function(data1){
							...
							{
								row1=value1.rows[j];
								if(row1.portNumber==portNumber&&row1.dpNumber==obj.id)
								{
	...
	//div.className="hello";
	div.id="queueGra";
	div.innerHTML=obj.id+"端口流量状况如下：（单位：KB）"
	div.style.border="1px solid #999999";
	div.style.display="none";
	
	topoDiv.append(div);
	$("#queueGra").slideDown("slow");

	div1=$("#queueGra");
	var queuediv=document.createElement("div");
	queuediv.style.width=600;
	queuediv.style.height=200;
	queuediv.style.position="absolute";
	queuediv.style.bottom="0px";
	queuediv.style.right="100px";
	
	
	queuediv.className="hello";
	queuediv.id="myDiv";
	div1.append(queuediv)
	if((queueDatas.length==0)&&(portRates.length!=0))
	{
		   // document.getElementById("queueGra").innerHTML=obj.id+"端口流量分如下：（单位：KB）";
		
					
			var a=(1/portRates.length*0.95).toFixed(4);
			...
			for(k=0;k<portRates.length;k++)
				{
			   
					var id="childDiv_0"+k;
				    myDiv=document.getElementById("myDiv");
					$("#myDiv").append("<div id="+id+" style='border-style:solid;border-width:1px;float:left; width:"+childwidth+";height:200px;'>")
					.attr("transform","translate(0,0)") ;
					var childDiv=document.getElementById(id);
					childDiv.innerHTML="port"+portNumbers[k];
					//var d=parseInt((portRates[k]);
	...
	{
			//document.getElementById("queueGra").innerHTML=obj.id+"端口划分队列情况如下：（单位：KB）";		
...
{
	var div = document.getElementById("queueGra");
...
Topology.prototype.update=function(){
  ...
  link.enter().insert("svg:line", "g.node")
	 ...
	 .style("stroke",function(d)
							
							  {
								  ...
								  if(d.source.id=="核心网"||d.target.id=="核心网")
								  d.color="black";
								  return d.color;
								  })
  ...
  nodeEnter.append("svg:image")
      ...
      .attr("xlink:href", function(d){
         //根据类型来使用图片
		 if(d.id=="核心网") return "img/cloud.png";
		 else
	  ...
	  .on('mouseover',
		  function(d){
			  if(d.id!="核心网"&&d.id!="流量识别器")creatDiv(d);})
...
var nodes=[
		...
		{id:'核心网',type:'switch',status:1,expand:true},
		{id:'流量识别器',type:'switch',status:1,expand:true}
	...
	var links=[
			 {source:'核心网',target:'流量识别器'}, 
			 {source:'流量识别器',target:'88-88-88-88-88-86'}
...
function showPortTable(data){
	...
	//var ip ="http://172.17.17.66/TopoShow/";
...
function initTopologyShow(){
	...
	//nodes.length=0;
	//links.length=0;
	var toposvgs=document.getElementById("TopologyShow").getElementsByTagName("svg");
	for(var i = 0 ;i<toposvgs.length;i++)
						{
				toposvgs.item(i).remove();
				}
	//toposvg.clear();
		...
		success:function(data){
			...
			for (i = 0; i < value.tableSize; i++) {
				 ...
				 if (row.linkStatus=="1") 
				 {
				 	...
				 	nodes.push(dstNode);
					if(portRate<700) color="blue";
					else if(portRate>700&&portRate<1500) color="green";
					else color="red";
...
function showSrcQueue(srcQueueRows,srcDp){
	...
	for(i=0;i<srcQueueRows.length;i++)
	{
		...
		else if((parseInt(srcQueueRows[i].queueRate)>500)&&(parseInt(srcQueueRows[i].queueRate)<1000))
		{
			var thisRow = "<tr>"
							  + "<td>"+ srcQueueRows[i].portNumber+"</td>"
							  + "<td>"+ srcQueueRows[i].queueNumber+"</td>"
	...
	for(i=0;i<dstQueueRows.length;i++)
		{
			...
			else if((parseInt(dstQueueRows[i].queueRate)>500)&&(parseInt(dstQueueRows[i].queueRate)<1000))
				{
					var thisRow = "<tr>"
									  + "<td>"+ dstQueueRows[i].portNumber+"</td>"
									  + "<td>"+ dstQueueRows[i].queueNumber+"</td>"
